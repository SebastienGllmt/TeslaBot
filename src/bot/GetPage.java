...
public class GetPage {
	...
	public GetPage(String url) throws IOException {
		...
		if (url.contains("youtube")) {
			...
			tweet = "";
			Matcher messageMatch = TITLE_TAG.matcher(content);
			if (messageMatch.find()) {
				tweet = messageMatch.group(1).replaceAll("[\\s\\<]+", " ").trim();
	 ...
	 * @param conn
	 * @return ContentType object representing the value of the Content-Type header
	 */
